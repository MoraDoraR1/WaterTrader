...
// 총 14단계로 15레벨을 구성한다. 전투/교역은 "시전" 횟수가 사실상 무한(같은 스킬을 몇 번이든
// 다시 쓸 수 있다)이라 이렇게 가파르게 잡아도 시간을 들이면 언젠가는 만렙에 닿는다.
...
// 학문(고고학/지리학/천문학) 전용 곡선 — 위 SKILL_EXP_CURVE를 그대로 재사용하면 절대
// 만렙에 닿을 수 없다. 학문은 "발견"으로만 크는데 도감 자체가 유한(22~24항목)해서, 그
// 유한한 발견 보상(rewardForRank)을 전부 더해도 SKILL_EXP_CURVE가 요구하는 누적치(레벨15까지
// 총 2236)의 6분의 1도 안 된다 — 실제로 도감을 전부 발견해도 레벨 9~10 언저리에서 더 이상
// 오를 곳이 없어 영구히 막히는 병목이 있었다(레벨10 이상을 요구하는 나머지 사이트들을 절대
// 열 수 없었다). 유한한 보상 풀 안에서도 완주 가능하도록 훨씬 완만하게 잡되, "1~8 빠르게 →
// 9~11 느리게 → 12~13 더 느리게 → 14~15 매우 느리게"라는 동일한 4단계 완급 형태는 그대로
// 유지한다(총합 248 — 도감 전체를 실제로 발견하며 시뮬레이션해 막힘없이 15까지 닿음을 확인).
export const ACADEMIC_EXP_CURVE = [
  2, 3, 4, 5, 6, 7, 9,   // 레벨 1→8 (7단계, 빠르게)
  12, 16, 20,            // 레벨 8→11 (3단계, 점점 느리게)
  26, 34,                // 레벨 11→13 (2단계, 더 느리게)
  44, 60,                // 레벨 13→15 (2단계, 매우 느리게)
];

const ACADEMIC_CATEGORIES = new Set(['archaeology', 'geography', 'astronomy']);

// id(스킬 id)에 맞는 레벨 곡선을 고른다 — 학문 3종만 ACADEMIC_EXP_CURVE, 나머지(전투/교역)는
// 기존 SKILL_EXP_CURVE.
export function curveFor(id) {
  return ACADEMIC_CATEGORIES.has(PLAYER_SKILLS[id]?.category) ? ACADEMIC_EXP_CURVE : SKILL_EXP_CURVE;
}
